...
public class SyntaxTree {
    ...
    public SyntaxTree(List<String> symbolTable) {
        loadKeywords();
        ...
        while (!aux.isEmpty()) {
            ...
            if (contains(token, COMANDOS_DE_BUSCA, 2)) {
                ...
                if (!aux.isEmpty()) {
                    ...
                    if (contains(token, FILTRO_DE_DATA, 2) && !aux.isEmpty()) {
                    ...
                    } else if (contains(token, FILTRO_DE_TAMANHO, 1) && !aux.isEmpty()) {
                        ...
                        System.err.println("TERMO ESPERADO");
                        break;
                    } else {
                        dunno();
                        break;
    ...
    private void loadKeywords() {
        try {
            Files.lines(Paths.get("./util/reserveds/searchinputs")).map(String::toLowerCase).forEach(COMANDOS_DE_BUSCA::add);
            Files.lines(Paths.get("./util/reserveds/pathinputs")).map(String::toLowerCase).forEach(COMANDOS_DE_DIRETORIO::add);
            Files.lines(Paths.get("./util/reserveds/dateinputs")).map(String::toLowerCase).forEach(COMANDOS_DE_DATA::add);
            Files.lines(Paths.get("./util/reserveds/sizeinputs")).map(String::toLowerCase).forEach(COMANDOS_DE_TAMANHO::add);

            Files.lines(Paths.get("./util/reserveds/extensions")).map(String::toLowerCase).forEach(EXTENSOES::add);
            Files.lines(Paths.get("./util/reserveds/relativedate")).map(String::toLowerCase).forEach(DATA_RELATIVA::add);

            Files.lines(Paths.get("./util/reserveds/datefilter")).map(String::toLowerCase).forEach(FILTRO_DE_DATA::add);
            Files.lines(Paths.get("./util/reserveds/sizefilter")).map(String::toLowerCase).forEach(FILTRO_DE_TAMANHO::add);
            Files.lines(Paths.get("./util/reserveds/pathfilter")).map(String::toLowerCase).forEach(FILTRO_DE_DIRETORIO::add);
            Files.lines(Paths.get("./util/reserveds/sizeunits")).map(String::toLowerCase).forEach(UNIDADES_TAMANHO::add);
        } catch (IOException e) {
